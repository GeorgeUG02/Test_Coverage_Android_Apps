...
class SearchViewModelTest {
    ...
    @Test
    fun liveData_TestReturnValueIsError2(){
        val observer = Observer<ScreenState> {}
        val liveData = searchViewModel.subscribeToLiveData()
        ...
        try {
            ...
            val value: ScreenState.Error = liveData.value as ScreenState.Error
            Assert.assertEquals(value.error.message, ERROR_TEXT2)
        } finally {
            liveData.removeObserver(observer)
        }
    }
    @Test
    fun liveData_TestReturnValueSuccess() {
        val observer = Observer<ScreenState> {}
        val liveData = searchViewModel.subscribeToLiveData()
    ...
    companion object {
        ...
        private const val ERROR_TEXT2="Search results or total count are null"
    }
}
